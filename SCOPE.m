...
path_of_code                = cd;
parameter_file              = {'input_data.xls'};

N                           = xlsread(['../' char(parameter_file)],'options');
...
options.Fluspect_model      = N(10);    %0: Fluspect 2010; 1: Fluspect 2014;
options.calc_rss_rbs        = N(11);    % 0: calculated from specific heat and conductivity (default), 1: empiricaly calibrated, 2: G as constant fraction of soil net radiation
options.apply_T_corr        = N(12);    % correct Vcmax and rate constants for temperature in biochemical.m
options.verify              = N(13);
options.save_headers        = N(14);    % write headers in output files
options.makeplots           = N(15);
options.simulation          = N(16);    % 0: individual runs (specify all input in a this file)
...
iter.maxit           = 150;                          %                   maximum number of iterations
            ...
            case 1
                ...
                if options.calc_fluor
                    if options.calc_vert_profiles
                        [rad,profiles] = RTMf(spectral,rad,soil,leafopt,canopy,gap,angles,profiles,leafbio,1);
                    else
                        [rad] = RTMf(spectral,rad,soil,leafopt,canopy,gap,angles,profiles,leafbio,0);
                    end
                end
