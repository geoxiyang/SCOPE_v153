function [iter,fluxes,rad,thermal,profiles]             ...  
...
while CONT                          % while energy balance does not close
    ...
    [lEch,Hch,ech,Cch]     = heatfluxes(raa+rawc,rcwh,Tch,ea,Ta,e_to_q,0,Ca,Cih);
    [lEcu,Hcu,ecu,Ccu]     = heatfluxes(raa+rawc,rcwu,Tcu,ea,Ta,e_to_q,0,Ca,Ciu);
    [lEs,Hs]               = heatfluxes(raa+raws,rss ,Ts ,ea,Ta,e_to_q,PSIs,Ca,Ca);
...
lEctot          = LAI*(Fc*lEch + meanleaf(canopy,lEch,'angles_and_layers',Ps)); % latent heat leaves
