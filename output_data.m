...
fprintf(fidr,'%9.0f  %9.0f %9.4f % 9.2f %9.2f',[k xyt.year(k) xyt.t(k) rad.Eouto rad.Eoutt + rad.Eoutte ]);
fprintf(fidr,'\r');
...
if options.calc_vert_profiles
    ...
    if options.calc_ebal
        ...
        fidhl           = fopen([Output_dir,'layer_H.dat'],'a');                    % [XY] h was changed to H for the use in Mac
        fprintf(fidhl,'%9.2f',[profiles.Hc1d' fluxes.Hstot]);
        fprintf(fidhl,'\r');
        
        fidlel          = fopen([Output_dir,'layer_lE.dat'],'a');                   % [XY] le was changed to lE for the use in Mac
        fprintf(fidlel,'%9.2f',[profiles.lEc1d' fluxes.lEstot]);
        fprintf(fidlel,'\r');
        
        fidal           = fopen([Output_dir,'layer_A.dat'],'a');                    % [XY] a was changed to A for the use in Mac
        ...
        fidrn           = fopen([Output_dir,'layer_Rn.dat'],'a');                   % [XY] rn was changed to Rn for the use in Mac
